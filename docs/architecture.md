...
`policy/rule-parity.yaml` is the migration control plane, not a JavaScript rule
engine. It pins the exact legacy source commit and raw-file digests, projects
all 101 source rules into content digests, and maps their 27 distinct source
keys into 23 nils capabilities, two stronger DSH-native seams, and one
provider-obsolete retirement. Planned groups name their future nils owner and
test owner but do not become dispatchable merely by appearing in the
inventory. The nils `DshCapabilityGroup` schema independently freezes the 23
deterministic IDs; the cross-repository verifier requires the two ordered sets
to match.
...
allows only the fixed inspection surface (`read`, `grep`, `glob`, image read,
runtime context, skills, bounded web reads, and agent listing) and denies every
other name, including Bash, write/edit/replace, code mode, nested tool calls,
subagent delegation, and recursive specialist review. DSH evaluates this guard
...
Local interrupted finalization additionally requires the installed tree to
match the digest derived from the reviewed archive. A registry interruption
without a precomputed content identity remains ambiguous rather than being
accepted by version alone.
