...
10. Source complete, release pending: the parity inventory has no planned
    active group, the DSH policy and package tree contain no retired handler
    rule or executable, and nils rejects `runtime-kit.handler.v1` for DSH while
    retaining that compatibility capability for supported Codex/Claude policy.
