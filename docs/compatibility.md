...
| Artifact routing and portable path hook | DSH provides session-owned `artifact_*` tools. The released nils-cli v1.28.42 native `portable-paths-scan` group also rejects hand-built `agent-out` directories and repo-local `.cache` scratch for DSH; the rule is not reimplemented in JavaScript. |
