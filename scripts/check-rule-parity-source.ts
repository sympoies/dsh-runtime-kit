...
const DSH_POLICY = testOwner(DSH, 'test/policy-parity.test.ts', PLANNED)
...
const PROVIDER_RETIREMENT = testOwner(DSH, 'test/provider-retirements.test.ts', ACTIVE)
...
const EXPECTED_SOURCE = {
  ...
  normalized_rule_id_digest: 'sha256:f715200f4f9b4474c29b12bd2fa46dfa881779e1e5e44d454ec5a3247284e4e8',
  rule_count: 102,
...
const EXPECTED_DISPOSITIONS = new Map(Object.entries({
  ...
  'session-start-healthcheck': disposition('operations.health.v1', 'implemented', DSH, [
    testOwner(DSH, 'test/runtime-health.test.ts', ACTIVE),
  ...
  'user-prompt-agent-docs': disposition('context.selective.v1', PLANNED, `${DSH} + ${NILS}`, [
    testOwner(DSH, 'test/context.test.ts', PLANNED),
