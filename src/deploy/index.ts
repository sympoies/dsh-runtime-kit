...
function usage(code: string, message: string, details: Record<string, unknown> = {}) {
...
export function parseScope(argv: string[], env: NodeJS.ProcessEnv): {help: true} | {help: false, scope: DeployScope}  {
  let parsed
  try {
    parsed = parseArgs({
      args: argv,
...
function runEngine(scope: DeployScope, packageRoot: string | undefined, engine: ReturnType<typeof resolveEngine>, io: {execPath: string, env: NodeJS.ProcessEnv}) {
  ...
  const args = [
    ...
    ...(scope.apply ? ['--apply', '--expected-plan-digest', ((scope.expectedPlanDigest) as string)] : []),
    '--format', 'json',
  ]
...
function writeReceipt(path: string, value: unknown) {
  try {
...
export async function main(argv: string[] = process.argv.slice(2), io: {env?: NodeJS.ProcessEnv, stdout?: {write: (text: string) => unknown}, execPath?: string, projectRoot?: string} = {}): Promise<number>  {
  ...
  try {
    ...
    if (scope.artifactPath !== undefined) {
      staged = await stageArtifact(scope.artifactPath, ((scope.artifactSha256) as string), scope.stageRoot)
    }
