...
async function observeQuiescence(handle: SubprocessHandle, timeoutMs: number) {
  let timer
  try {
...
async function runHealthCommand(ctx: Context, argv: string[], cwd: string, signal: AbortSignal, quiescenceMs: number, beforeSpawn: (() => Promise<void>) | undefined, spawn: ((spec: Record<string, unknown>) => SubprocessHandle) | undefined) {
  ...
  try {
    ...
    return Object.freeze({
      outcome: ((outcome) as {exitCode: number | null, signal: string | null}),
      stdout: stdout.text,
    })
  } finally {
...
function fingerprint(value: import('node:fs').Stats) {
  return Object.freeze({
    dev: value.dev,
    ino: value.ino,
    mode: value.mode,
...
function sameFingerprint(left: ReturnType<typeof fingerprint>, right: ReturnType<typeof fingerprint>) {
...
async function assertTrustedExecutableAncestors(path: string) {
...
async function inspectAuthenticatedBinary(path: string, expected: string, retainBytes: boolean) {
...
async function readAuthenticatedBinary(path: string, expected: string) {
  return ((await inspectAuthenticatedBinary(path, expected, true)) as Awaited<ReturnType<typeof inspectAuthenticatedBinary>> & {bytes: Buffer})
}

async function assertBinaryUnchanged(binary: {handle: import('node:fs/promises').FileHandle, identity: ReturnType<typeof fingerprint>}) {
...
async function assertTrustedSnapshotRoot(directory: string) {
...
async function pathReferencesOpenDirectory(path: string, rootHandle: import('node:fs/promises').FileHandle) {
  try {
...
async function pathReferencesOpenFile(path: string, handle: import('node:fs/promises').FileHandle) {
  try {
...
export async function snapshotExecutable(root: string, name: string, bytes: Buffer, expected: string, openFile: typeof open = open) {
...
export function createSnapshotExecutionOwner(
  ...
  spawn: ((spec: Record<string, unknown>) => SubprocessHandle) | undefined,
  warn: (message: string) => void = message => { process.emitWarning(message) },
) {
  let closing = false
  const active: Map<symbol, {controller: AbortController, settled: Promise<void>}> = new Map()
  const scopes: Map<symbol, Promise<void>> = new Map()
  let disposal: Promise<void> | undefined

  function acquire(signal: AbortSignal, leases: Map<symbol, {controller: AbortController, settled: Promise<void>}>) {
  ...
  function acquireDirect(signal: AbortSignal) {
    if (closing) {
  ...
  return Object.freeze({
    ...
    async run<T>(signal: AbortSignal, execute: (lease: ReturnType<typeof acquireDirect>) => Promise<T>) {
...
async function runAuthenticatedHealthCommand(
  ...
  ctx: Context,
  binary: Awaited<ReturnType<typeof snapshotExecutable>>,
  argv: string[],
  cwd: string,
  signal: AbortSignal,
  quiescenceMs: number,
  beforeSpawn: (() => Promise<void>) | undefined,
) {
...
function authenticatedDescriptorSpawner(ctx: Context, binaries: Map<string, {handle: import('node:fs/promises').FileHandle, identity: ReturnType<typeof fingerprint>}>, binding: {service: any, mode: string, spawnDescriptor: Function}) {
  return (spec: Record<string, any>) => {
    ...
    const tracedSubprocess = ((ctx.subprocess) as any)
...
function blocked(error: unknown, signal: AbortSignal, fallback: string) {
  if (signal.aborted) throw abortReason(signal)
  return {
...
function validDshRuntime(value: unknown) {
...
function validAudit(value: unknown, docsHome: string, projectPath: string) {
...
function ownRegistration(ctx: Context, dispose: () => void, label: string) {
...
export async function installNilsHealthProviders(ctx: Context, health: import('./index.js').RuntimeHealth, config: {agentHook?: string, agentHookConfig?: string, agentHookPolicy?: string, agentHookStateDir?: string, agentDocs?: string, agentDocsHome?: string, agentDocsStateHome?: string, nilsCompatibilityCandidate?: string, [key: string]: unknown}, options: {compatibility?: unknown, dshRuntime: unknown, childPlugins: {main_agent_mode: {state:string}, review_specialists: {state:string}}, commandQuiescenceMs?: number, beforeCommandSpawn?: () => Promise<void>}) {
  ...
  const subprocess = originalCordisService(((ctx.subprocess) as any))
  ...
  const disposeRuntime = health.register({
    ...
    async probe({ signal }) {
      try {
        ...
        return { state: (('ready') as const), code: 'DSH_RUNTIME_HEALTH_RUNTIME_READY' }
      } catch (error) {
    ...
    async probe({ scope, signal }) {
      try {
        ...
        if (audit.problems > 0 && result.outcome.exitCode === 1 && result.outcome.signal === null) {
          return { state: (('blocked') as const), code: 'DSH_RUNTIME_HEALTH_PROJECT_INVALID' }
        }
        if (audit.problems !== 0 || result.outcome.exitCode !== 0 || result.outcome.signal !== null) {
          throw new HealthProbeFailure('DSH_RUNTIME_HEALTH_PROJECT_AUDIT_INVALID')
        }
        return { state: (('ready') as const), code: 'DSH_RUNTIME_HEALTH_PROJECT_READY' }
      } catch (error) {
  ...
  for (const [capability, name] of childCapabilities) {
    const dispose = health.register({
      ...
      probe() {
        ...
        if (state === 'active') {
          return { state: (('ready') as const), code: 'DSH_RUNTIME_HEALTH_OPTIONAL_READY' }
        }
        if (state === 'pending') {
          return { state: (('degraded') as const), code: 'DSH_RUNTIME_HEALTH_OPTIONAL_PENDING' }
        }
        return { state: (('degraded') as const), code: 'DSH_RUNTIME_HEALTH_OPTIONAL_UNAVAILABLE' }
      },
    })
