...
export function validateDshCompatibilityManifest(input: unknown) {
  ...
  for (const [name, value] of Object.entries(packages)) {
    ...
    const peerRange = ((DSH_RC7_PEER_RANGES) as Record<string, string>)[name]
    ...
    const runtimeExports = ((DSH_RC7_RUNTIME_MODULES) as Record<string, Record<string, string>>)[name] ?? {}
...
function matchesKind(value: unknown, kind: string) {
  if (kind === 'object') return value !== null && typeof value === 'object' && !Array.isArray(value)
  return typeof value === kind
}

async function resolvedPackageVersion(specifier: string) {
  let cursor
  try {
...
export async function loadDshRc7Runtime(options: {importModule?: (specifier: string) => Promise<Record<string, unknown>>, packageVersion?: (specifier: string) => Promise<string | undefined>} = {}) {
  ...
  for (const [specifier, expectedVersion] of Object.entries(DSH_RC7_PEER_RANGES)) {
    ...
    if (!supported) {
      const diagnosticVersion = ((DSH_RC7_PEER_VERSIONS) as Record<string, string>)[specifier] ?? expectedVersion
  ...
  if (missing.length === 0 && selectedDshVersions.size !== 1) {
    const retainedPeerVersions = ((DSH_RC7_PEER_VERSIONS) as Record<string, string>)
  ...
  if (missing.length === 0) {
    const selectedDshVersion = ((selectedDshVersions.values().next().value) as string)
    const expectedCordisVersion = ((SUPPORTED_DSH_RELEASES) as Record<string, {cordis: string}>)[selectedDshVersion].cordis
...
async function canonicalContainedPath(root: string, candidate: string) {
  let canonical
  try {
