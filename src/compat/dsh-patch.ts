...
function validHashPair(value: unknown) {
...
export function validateDshPatchManifest(value: unknown) {
...
function targetHashes(target: Record<string, any>, release: string) {
...
async function trustedGit(gitBin: string) {
...
function execFileWithInput(file: string, args: string[], options: import('node:child_process').ExecFileOptionsWithStringEncoding, input: Buffer): Promise<{stdout: string, stderr: string}>  {
...
async function git(gitBin: string, cwd: string, args: string[], input?: Buffer): Promise<string>  {
  try {
    const argv = [
      '--no-optional-locks',
...
function indexEntries(output: string) {
...
function headEntries(output: string) {
  const entries = new Map()
  for (const record of output.split('\0').filter(Boolean)) {
...
function gitBlobDigest(bytes: Buffer, algorithm: 'sha1'|'sha256') {
...
function validatePatchTargets(bytes: Buffer, expected: string[]) {
  let text
  try {
  ...
  const exact = (values: string[]) => values.length === new Set(values).size
...
export async function manageDshPatch(input: {
  ...
  const inspect = async () => {
    const states = await Promise.all(Object.entries(selectedTargets).map(async ([path, hashes]) => {
      ...
      if (actual === hashes.after_sha256) return (('patched') as const)
      throw new DshPatchError(
        'DSH_RUNTIME_KIT_DSH_PATCH_DRIFT',
