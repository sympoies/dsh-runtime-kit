...
function denseArray(value: unknown) {
...
function includesAll(actual: readonly string[], required: readonly string[]) {
  const available = new Set(actual)
  return required.every(value => available.has(value))
}

function excludesAll(actual: readonly string[], forbidden: readonly string[]) {
  const available = new Set(actual)
  return forbidden.every(value => !available.has(value))
}

function hasExactKeys(value: Record<string, any>, expected: readonly string[]) {
...
export function inspectAgentConsoleRc7Profile(observation: unknown) {
  ...
  const disabledRows = stringArray(composition?.disabledRowIds)
  const enabledRequiredRows = CONTRACT.required_rows.filter(
    (row: string) => !CONTRACT.required_disabled_rows.includes(row),
  )
  if (disabledRows === undefined
    || !includesAll(disabledRows, CONTRACT.required_disabled_rows)
    || !excludesAll(disabledRows, enabledRequiredRows)) {
    fail(
      'DSH_RUNTIME_KIT_AGENT_CONSOLE_DISABLED_ROW_MISMATCH',
      'dsh-runtime-kit: Agent Console legacy rows or required services have the wrong activation state',
    )
  }
  ...
  const sandboxPair = CONTRACT.authority.sandbox_approval_pairs.some(
    (pair: any) => pair.sandbox_mode === authority?.sandboxMode
