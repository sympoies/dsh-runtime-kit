...
async function trustedGit(path: string) {
  let canonical
  let metadata
  try {
...
async function git(gitBin: string, sourceRoot: string, args: string[]) {
  try {
...
async function selectedCheckoutState(input: {sourceRoot: string, channel: string, gitBin: string, manifest: unknown}) {
