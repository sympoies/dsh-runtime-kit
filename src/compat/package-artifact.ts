...
async function ensureRealDirectory(path: string) {
  ...
  try {
    ...
    if (((error) as NodeJS.ErrnoException).code !== 'ENOENT') throw unsafeInstallScope()
    try {
...
async function assertDirectoryIdentity(identity: {path: string, dev: number, ino: number}) {
  let info
  let canonical
  try {
...
export async function prepareAuthenticatedPackageScope(consumerRoot: string, scopeName: string) {
  ...
  const resolveTarget = (base: string) => {
...
function textField(header: Buffer, offset: number, length: number) {
...
function octalField(header: Buffer, offset: number, length: number) {
...
function parsePackageArtifact(tarball: Buffer) {
...
export async function extractPackageArtifact(tarball: Buffer, targetRoot: string, options: {afterTargetOpened?: () => Promise<void>} = {}) {
  ...
  const directories: Array<{handle: import('node:fs/promises').FileHandle, path: string, dev: number, ino: number}> = []
  const directoryByPath: Map<string, typeof directories[number]> = new Map()
  const files: Array<{parent: typeof directories[number], name: string, dev: number, ino: number, mode: number, bytes: Buffer}> = []
  try {
