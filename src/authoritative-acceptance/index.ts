...
const lexicalCompare: (left: string, right: string) => number = (left, right) => left < right ? -1 : left > right ? 1 : 0
...
export class DshAcceptanceBlockedError extends Error {
  ...
  constructor(aggregate: string, reasonCodes: readonly string[] = []) {
...
function temporaryProviderError(error: unknown) {
  return error !== null && typeof error === 'object'
    && ((error) as {code?: unknown}).code === 'DSH_FINISH_LINE_TEMPORARY'
}
...
function definitionProjection(definition: ToolDefinition) {
...
function identifier(value: unknown, what: string) {
...
function normalizeRegistration(input: unknown) {
  ...
  const validatorBindings: Map<ToolDefinition, readonly any[]> = new Map()
  const toolDefinitionsByName: Map<string, ToolDefinition> = new Map()
  const definitionDigests: Map<ToolDefinition, string> = new Map()
  const requirements = value.requirements.map(rawRequirement => {
    ...
    const validators = requirement.validators.map(rawValidator => {
      ...
      const definition = ((validator?.definition) as ToolDefinition)
      ...
      if (execution.kind === 'host-observed' && Object.keys(execution).length === 1) {
        normalizedExecution = Object.freeze({ kind: (('host-observed') as const) })
...
function wireRegistration(contract: ReturnType<typeof normalizeRegistration>) {
  return {
...
export function createAcceptanceProjection(contract: ReturnType<typeof normalizeRegistration>) {
  ...
  return Object.freeze({
    ...
    apply(state: {active: Array<{callId: string, kind: 'mutation'|'validator', name: string}>, last: null | {kind: 'mutation'|'validator', name: string, status: 'succeeded'|'failed'}}, event: any) {
...
function sanitizeVerdict(input: unknown) {
  const value = plainRecord(input)
  if (value === undefined
    || !['allow', 'block'].includes(((value.action) as string))
    || !VERDICT_STATUSES.has(((value.aggregate) as string))
  ...
  if (rawReservation !== undefined
    && (completionReservation === undefined
      ...
      || !['reserved', 'duplicate'].includes(
        ((completionReservation.status) as string),
      ))) {
    throw new Error('dsh-runtime-kit: acceptance verdict invalid')
  }
  if (new Set(names).size !== names.length
    ...
    || (value.action === 'allow'
      ...
      : reasonCodes.length === 0 || !reasonCodes.includes(((value.aggregate) as string)))) {
    throw new Error('dsh-runtime-kit: acceptance verdict invalid')
  }
  return Object.freeze({
    action: ((value.action) as 'allow' | 'block'),
    aggregate: ((value.aggregate) as string),
    generation: ((value.generation) as number),
    contractDigest: ((value.contractDigest) as string),
    ...
    ...completionReservation === undefined
      ...
      : {
          completionReservation: Object.freeze({
            operationId: ((completionReservation.operationId) as string),
            status: ((completionReservation.status) as 'reserved' | 'duplicate'),
          }),
        },
  })
}

function observedStatus(result: ToolExecutionResult, abortedCode: string) {
...
export function createAuthoritativeAcceptanceCoordinator(ctx: Context, options: {
  ...
  const controlTimeoutMs = typeof options.controlTimeoutMs === 'number'
    ...
    : 2_000
  let contract: ReturnType<typeof normalizeRegistration> | undefined
  let sessions: WeakMap<Agent['session'], SessionState> = new WeakMap()
  let stopSteers: WeakMap<Agent['session'], {turn: number, count: number}> = new WeakMap()
  const liveStates: Set<SessionState> = new Set()
  const operations: Map<Readonly<ToolExecution>, OperationState> = new Map()
  const repositoryMutations: Map<Readonly<ToolExecution>, string> = new Map()
  const repositoryPreparations: Map<string, Promise<void>> = new Map()
  const readinessWaits: Set<Promise<void>> = new Set()
  const lifecycleAbort = new AbortController()
  let open = true

  function hasRepositoryMutation(repositoryKey: string) {
 ...
 */
  ...
  async function withControlSignal<T>(callerSignal: AbortSignal, invoke: (signal: AbortSignal) => Promise<T>, includeLifecycle: boolean = true): Promise<T>  {
    const controller = new AbortController()
    const signals = includeLifecycle
      ? [callerSignal, lifecycleAbort.signal]
      : [callerSignal]
    const listeners: Map<AbortSignal, () => void> = new Map()
    const abortFrom = (signal: AbortSignal) => controller.abort(
 ...
 */
  ...
  async function awaitWorkspaceReady(agent: Agent, callerSignal?: AbortSignal) {
  ...
  function isWorkspaceReady(agent: Agent) {
    try {
 ...
 */
  ...
  function operationObservation(operation: OperationState, status: string | undefined) {
    ...
    return { kind: (('host-observed') as const), status }
  }
 ...
 */
  ...
  async function retryProvider<T>(signal: AbortSignal, invoke: (signal: AbortSignal) => Promise<T>, includeLifecycle: boolean = true): Promise<T>  {
  ...
  function assertLive(agent: Agent) {
  ...
  async function awaitPublication(agent: Agent) {
 ...
 */
  ...
  function poison(state: SessionState, error: unknown) {
    state.revision += 1
 ...
 */
  ...
  function steerStop(agent: Agent, turn: number, text: string) {
  ...
  function invalidate(state: SessionState) {
    state.revision += 1
    state.verdict = Object.freeze({
      action: (('block') as const),
      aggregate: 'active',
  ...
  function track(state: SessionState, promise: Promise<unknown>) {
  ...
  function trackCompletion(state: SessionState, promise: Promise<unknown>, successStatus: 'succeeded' | 'cancelled' | undefined, preserveRejection: boolean = false) {
  ...
  async function refresh(state: SessionState, turnId: string, signal: AbortSignal, reserveCompletion: boolean = false) {
  ...
  async function releaseCompletion(state: SessionState, turnId: string, signal: AbortSignal, operationId: string, status: string) {
  ...
  function runRefresh(state: SessionState, turnId: string, signal: AbortSignal, reserveCompletion: boolean = false) {
  ...
  async function registerState(state: SessionState, signal: AbortSignal) {
  ...
  function stateFor(agent: Agent) {
    ...
    const created: SessionState = {
      ...
      repositoryKey: ((agent.session.header.cwd) as string),
  ...
  function assertContractBindings(agent: Agent) {
    if (contract === undefined) return
  ...
  async function ensureRegistered(agent: Agent, signal: AbortSignal = new AbortController().signal) {
  ...
  function bindingFor(exec: Readonly<ToolExecution>) {
    ...
    if (validators !== undefined) {
      ...
      if (hostObserved !== undefined) {
        return { kind: (('validator') as const), ...hostObserved }
      }
    ...
    return invalidator === undefined
      ? undefined
      : { kind: (('mutation') as const), ...invalidator }
  }

  async function observe(operation: OperationState, observation: any) {
 ...
 */
  ...
  async function prepareRepositoryMutation(exec: Readonly<ToolExecution>, call: {turn: number}) {
    if (contract === undefined) return
      ...
      if (typeof ctx.inject === 'function') {
        ctx.inject(['sessionProjections'], projectionCtx => {
          const projections = /** @type {{register(definition: any): () => void}} */ (
            ((projectionCtx) as any).sessionProjections
          )
 ...
 */
    ...
    completionSettlement(agent: Agent) {
    ...
    assertGoalCompletion(agent: Agent, _ref: unknown) {
      if (contract === undefined) return
      assertLive(agent)
      const state = sessions.get(agent.session)
      try {
  ...
  const coordinator = Object.freeze({
    ...
    governs(_agent: Agent) { return contract !== undefined },

    async sessionStarted(payload: {agent: Agent, source: unknown}) {
      if (contract === undefined) return
      if (!open) throw new Error('dsh-runtime-kit: acceptance coordinator disposed')
    ...
    async repositoryMutationStarting(exec: Readonly<ToolExecution>, call: {turn: number}) {
 ...
 */
    ...
    async admit(exec: ToolExecution, call: {sessionId: string, cwd: string, turn: number, step?: number, callId: string, rootCallId: string, name: string}) {
      if (!open || contract === undefined) return { kind: (('none') as const) }
      const binding = bindingFor(exec)
      if (binding === undefined) return { kind: (('none') as const) }
      ...
      const admission = (async () => {
        ...
        const record: OperationState = { exec, state, binding, operationId, sourceOperationId, call }
        if (!open || state.disposed) {
    ...
    result(exec: Readonly<ToolExecution>, result: ToolExecutionResult) {
      repositoryMutations.delete(exec)
    ...
    reject(exec: Readonly<ToolExecution>, status: string = 'cancelled') {
      repositoryMutations.delete(exec)
      const operation = operations.get(exec)
      if (operation === undefined) return
      operations.delete(exec)
    ...
    async turnStopping(payload: {agent: Agent, turn: number, signal: AbortSignal}) {
 ...
 */
    ...
    async cancelCompletion(agent: Agent, turnId: string) {
      if (contract === undefined) return
      assertLive(agent)
      const state = sessions.get(agent.session)
    ...
    async agentDisposed(agent: Agent) {
      const state = sessions.get(agent.session)
      if (state === undefined) return
      state.disposed = true
      invalidate(state)
...
export type SessionState = { agent: Agent, session: Agent['session'], repositoryKey: string, contractDigest: string | undefined, verdict: ReturnType<typeof sanitizeVerdict> | undefined, poison: string | undefined, registration: Promise<void> | undefined, registrationPending: boolean, pending: Set<Promise<unknown>>, completionTasks: Set<Promise<unknown>>, admissions: Set<Promise<unknown>>, refreshes: Set<Promise<unknown>>, revision: number, refreshSequence: number, appliedRefreshSequence: number, activeOperations: number, completionReservationId: string | undefined, completionReservationActive: boolean, completionSettlement: 'idle' | 'reserved' | 'pending' | 'cancelling' | 'succeeded' | 'failed' | 'cancelled', disposed: boolean }

export type OperationState = { exec: Readonly<ToolExecution>, state: SessionState, binding: any, operationId: string, sourceOperationId: string | undefined, call: {sessionId: string, cwd: string, turn: number, step?: number, callId: string, rootCallId: string, name: string} }
