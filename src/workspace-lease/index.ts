...
export type PostToolDecision = import('@deepseek-ai/dsh-tools').PostToolDecision
export type PreToolDecision = import('@deepseek-ai/dsh-tools').PreToolDecision
...
export type WorkspaceLeaseState = 'owned' | 'foreign-active' | 'stale-clean' | 'dirty' | 'uncertain' | 'unavailable' | 'unmanaged'
export type WorkspaceLeaseReleaseReason = 'agent-disposed' | 'session-rebound' | 'provider-disposed'
export type WorkspaceLeaseOperationOutcome = 'succeeded' | 'failed' | 'cancelled'

export type WorkspaceRef = Readonly<Record<never, never>>
/**
...
export type WorkspaceLeaseResolveRequest = WorkspaceLeaseBindingFacts & {
    anchorCwd?: string,
...
export type WorkspaceLeaseBeginRequest = WorkspaceLeaseBindingFacts & {
    bindingId: string,
    workspaceId: string,
    generation: string,
...
export type WorkspaceLeaseBeginResult = {kind: 'not-required'}
    | {kind: 'granted', operationId: string, fence: string}
    | WorkspaceLeaseDenied

export type WorkspaceLeaseCompleteRequest = WorkspaceLeaseBindingFacts & {
    bindingId: string,
    workspaceId: string,
    generation: string,
    operationId: string,
...
export type WorkspaceLeaseRenewRequest = WorkspaceLeaseBindingFacts & {
    bindingId: string,
    workspaceId: string,
    generation: string,
  }

export type WorkspaceLeaseRenewResult = {kind: 'renewed', renewAfterMs?: number}
    | {kind: 'lost', state: Exclude<WorkspaceLeaseState, 'owned' | 'unmanaged'>, code: string, reason: string}

export type WorkspaceLeaseReleaseRequest = WorkspaceLeaseBindingFacts & {
    bindingId: string,
    workspaceId: string,
    generation: string,
...
export type WorkspaceLeaseProvider = { protocolVersion: typeof WORKSPACE_LEASE_PROTOCOL_VERSION, resolve: (request: WorkspaceLeaseResolveRequest, signal: AbortSignal) => Promise<WorkspaceLeaseResolveResult>, bind: (request: WorkspaceLeaseBindRequest, signal: AbortSignal) => Promise<WorkspaceLeaseBindResult>, begin: (request: WorkspaceLeaseBeginRequest, signal: AbortSignal) => Promise<WorkspaceLeaseBeginResult>, complete: (request: WorkspaceLeaseCompleteRequest, signal: AbortSignal) => Promise<void>, renew: (request: WorkspaceLeaseRenewRequest, signal: AbortSignal) => Promise<WorkspaceLeaseRenewResult>, release: (request: WorkspaceLeaseReleaseRequest, signal: AbortSignal) => Promise<void> }
/**
...
export type ProviderSlot = { provider: WorkspaceLeaseProvider, bindings: Set<BoundWorkspace>, stopping: boolean }

export type AgentSlot = { agent: Agent, session: Agent['session'], epoch: number, source: SessionStartSource, disposed: boolean, bindings: Map<string, BoundWorkspace>, acquisitions: Map<string, Promise<BoundWorkspace | undefined>>, lifecycle: AbortController, draining: Promise<void> | undefined, anchor: Promise<AnchorClassification> | undefined, anchorController: AbortController | undefined, anchorTarget: string | undefined, ref: WorkspaceRef | undefined }
/**
...
export type AnchorClassification = { state: 'owned' | 'unmanaged' }
/**
...
export type BoundWorkspace = { owner: AgentSlot, acquirer: AgentSlot, session: Agent['session'], provider: ProviderSlot, workspaceKey: string, bindingId: string, workspaceId: string, generation: string, target: WorkspaceLeaseTarget, initialState: 'owned' | 'unmanaged', lifecycle: AbortController, admissions: Set<Promise<void>>, operations: Set<LeaseOperation>, renewAfterMs: number | undefined, renewTimer: ReturnType<typeof setTimeout> | undefined, renewController: AbortController | undefined, renewing: Promise<void> | undefined, failure: WorkspaceLeaseError | undefined, released: boolean, releaseTask: Promise<void> | undefined }

export type LeaseOperation = { binding: BoundWorkspace, identity: ExecutionIdentity, operationId: string, fence: string, authority: AbortController, completion: AbortController, settled: Promise<void>, settle: () => void, completing: boolean }

export type ExecutionIdentity = { token: ToolExecution['token'], callId: ToolExecution['callId'], rootCallId: ToolExecution['rootCallId'], toolName: string, arguments: unknown, agent: Agent, session: Agent['session'], parent: ToolExecution['parent'], signal: AbortSignal }
/**
...
export type ExecutionAuthorization = ExecutionIdentity & {
    slot: AgentSlot,
    epoch: number,
...
export type WorkspaceRefMeta = { agent: Agent, slot: AgentSlot }

export type FusedSignal = { signal: AbortSignal, dispose: () => void }
...
export class WorkspaceLeaseError extends HarnessError {
  ...
  constructor(message: string, code: string, state: WorkspaceLeaseState, options?: ErrorOptions) {
...
function providerError(result: WorkspaceLeaseDenied) {
...
function assertProviderResult(value: unknown, operation: string) {
  if (value === null || typeof value !== 'object' || Array.isArray(value)) {
    throw unavailable(`workspace lease provider returned an invalid ${operation} result`)
  }
}

function assertProvider(value: unknown) {
  if (value === null || typeof value !== 'object') {
    throw unavailable('workspace lease provider is invalid')
  }
  const candidate = ((value) as Partial<WorkspaceLeaseProvider>)
  ...
  for (const method of ['resolve', 'bind', 'begin', 'complete', 'renew', 'release']) {
    if (typeof candidate[((method) as keyof WorkspaceLeaseProvider)] !== 'function') {
      throw unavailable(`workspace lease provider is missing ${method}()`)
    }
  }
}

function positiveDelay(value: number | undefined) {
...
function bindingFacts(session: Agent['session']): Omit<WorkspaceLeaseBindingFacts, 'requestId'>  {
  return {
...
function executionIdentity(exec: ToolExecution, agent: Agent): ExecutionIdentity  {
  return {
    token: exec.token,
    callId: exec.callId,
...
function matchesExecution(authorization: ExecutionIdentity, exec: Readonly<ToolExecution>) {
...
function deferred() {
  let resolve = () => {}
  const promise: Promise<void> = new Promise(accept => {
    resolve = () => { accept() }
  })
  return { promise, resolve }
}

function fuseSignals(signals: readonly AbortSignal[]): FusedSignal  {
...
function errorCode(result: ToolExecutionResult | undefined) {
...
export class WorkspaceLease extends Service {
  ...
  #provider: ProviderSlot | undefined
  #agentSlots: WeakMap<Agent, AgentSlot> = new WeakMap()
 ...
 */
  ...
  #admitting: WeakSet<ToolExecution> = new WeakSet()
 ...
 */
  ...
  registerProvider(provider: WorkspaceLeaseProvider): () => Promise<void>  {
    assertProvider(provider)
    return this.ctx.effect(() => {
      if (this.#provider !== undefined) {
        throw unavailable('a workspace lease provider is already registered')
      }
      const slot: ProviderSlot = { provider, bindings: new Set(), stopping: false }
      this.#provider = slot
 ...
 */
  ...
  async ref(agent: Agent): Promise<WorkspaceRef>  {
 ...
 */
  ...
  async targets(exec: ToolExecution): Promise<readonly string[]>  {
  ...
  #resolutionFor(exec: ToolExecution, provider: ProviderSlot, slot: AgentSlot, identity: ExecutionIdentity, signal: AbortSignal): Promise<readonly WorkspaceLeaseTarget[]>  {
 ...
 */
  ...
  async denialState(agent: Agent): Promise<{state: Exclude<WorkspaceLeaseState, 'owned' | 'unmanaged'>, code: string} | null>  {
    this.#assertLive(agent)
    const slot = this.#agentSlots.get(agent)
    ...
    try {
      ...
      if (error instanceof WorkspaceLeaseError
        ...
        return {
          state: ((error.state) as Exclude<WorkspaceLeaseState, 'owned' | 'unmanaged'>),
          code: error.code,
        }
      }
      return { state: 'unavailable', code: WORKSPACE_LEASE_UNAVAILABLE }
 ...
 */
  ...
  async state(agent: Agent, ref: WorkspaceRef): Promise<WorkspaceLeaseState>  {
  ...
  #requireRef(agent: Agent, ref: WorkspaceRef): AgentSlot  {
    this.#assertLive(agent)
 ...
 */
  ...
  #rebind(slot: AgentSlot) {
    ...
    const ref = ((Object.freeze(Object.create(null))) as WorkspaceRef)
    slot.ref = ref
 ...
 */
  ...
  #startAnchor(slot: AgentSlot): Promise<AnchorClassification> | undefined  {
    ...
    const anchor = (async () => {
      ...
      if (cwd === undefined) return { state: (('unmanaged') as const) }
  ...
  async #invokeBind(provider: ProviderSlot, request: WorkspaceLeaseBindRequest, signal: AbortSignal): Promise<WorkspaceLeaseBindResult>  {
    let result: WorkspaceLeaseBindResult
    try {
  ...
  async #releaseGeneration(provider: ProviderSlot, session: Agent['session'], result: WorkspaceLeaseBound, reason: WorkspaceLeaseReleaseReason) {
    try {
 ...
 */
  ...
  #adoptBinding(owner: AgentSlot, acquirer: AgentSlot, provider: ProviderSlot, session: Agent['session'], result: WorkspaceLeaseBound): BoundWorkspace  {
 ...
 */
  ...
  #admit(exec: ToolExecution, identity: ExecutionIdentity, slot: AgentSlot, epoch: number, operations: readonly LeaseOperation[]) {
 ...
 */
  ...
  #bindingUsable(binding: BoundWorkspace): boolean  {
 ...
 */
  ...
  #bindingCurrent(binding: BoundWorkspace): boolean  {
    return this.#bindingUsable(binding)
      && binding.owner.bindings.get(binding.workspaceKey) === binding
  }

  #admissionChanged(slot: AgentSlot, epoch: number, identity: ExecutionIdentity, exec: ToolExecution, bindings: readonly BoundWorkspace[], signal: AbortSignal) {
    if (slot.disposed
      || slot.epoch !== epoch
  ...
  #firstFailure(bindings: readonly BoundWorkspace[]) {
    for (const binding of bindings) {
 ...
 */
  ...
  async #execute(exec: ToolDispatchExecution, next: () => Promise<ToolExecutionResult>): Promise<ToolExecutionResult>  {
    ...
    try {
      ...
      fused.dispose()
      const outcome: WorkspaceLeaseOperationOutcome = fused.signal.aborted
        ? 'cancelled'
 ...
 */
  ...
  async #postExecute(exec: ToolExecution, result: Readonly<ToolExecutionResult>, next: () => Promise<PostToolDecision>): Promise<PostToolDecision>  {
    ...
    try {
      ...
      if (operations !== undefined && operations.length > 0) {
        const outcome: WorkspaceLeaseOperationOutcome = exec.signal.aborted
          ? 'cancelled'
  ...
  async #completeAll(exec: ToolExecution, operations: readonly LeaseOperation[], outcome: WorkspaceLeaseOperationOutcome, code?: string) {
  ...
  async #completeOperation(operation: LeaseOperation, outcome: WorkspaceLeaseOperationOutcome, code?: string) {
  ...
  #armRenewal(binding: BoundWorkspace) {
 ...
 */
  ...
  #syncRenewalTimerRef(binding: BoundWorkspace) {
  ...
  async #renewBinding(binding: BoundWorkspace) {
    ...
    let result: WorkspaceLeaseRenewResult
    try {
 ...
 */
  ...
  #forgetAnchor(binding: BoundWorkspace) {
 ...
 */
  ...
  #markLost(binding: BoundWorkspace, failure: WorkspaceLeaseError) {
  ...
  async #releaseAll(slot: AgentSlot, reason: WorkspaceLeaseReleaseReason): Promise<void>  {
  ...
  async #releaseBinding(binding: BoundWorkspace, reason: WorkspaceLeaseReleaseReason) {
