...
export function requiresTrustAcceptanceFailureDigest(code: string) {
  return TRUST_ACCEPTANCE_FAILURE_DIGEST_REQUIRED.has(code)
}

export function mapTrustAuthorityFailureCode(code: string) {
  return ((TRUST_FAILURE_CODE_MAP) as Record<string, string>)[code] ?? code
}
...
export function isManagerFailureCode(kind: string, code: string) {
  return ((MANAGER_FAILURE_CODES_BY_KIND) as Record<string, Set<string>>)[kind]?.has(code) === true
}
...
export function plainRecord(value: unknown, path: string) {
...
export function exactKeys(value: Record<string, any>, required: string[], optional: string[] = [], path: string = 'document') {
...
export function text(value: unknown, path: string, maximum: number = 4096) {
...
export function digest(value: unknown, path: string) {
...
export function uint64(value: unknown, path: string) {
...
export function utc(value: unknown, path: string) {
...
export function nonce(value: unknown, path: string) {
...
export function identity(value: unknown, path: string = 'identity') {
...
function sortedIdentities(value: unknown, path: string) {
...
export function assertSecretFree(value: unknown, path: string = 'document') {
  const seen = new Set()
  const walk = (item: unknown, itemPath: string, depth: number) => {
...
export function assertCanonicalByteBound(
  value: unknown,
  path: string,
  maximum: number = MANAGER_CONTROL_MAX_DATAGRAM_BYTES,
) {
...
function digestProjection(source: Record<string, any>) {
...
export function computeManagerDocumentDigest(value: unknown) {
  const document = plainRecord(value, 'document')
  const domain = ((RUNTIME_MANAGER_DOMAIN_TAGS) as Record<string, string>)[document.kind]
...
export function computeSemanticRequestDigest(value: unknown) {
  const request = plainRecord(value, 'request')
  const domain = ((RUNTIME_MANAGER_DOMAIN_TAGS) as Record<string, string>)[request.kind]
...
export function computeTrustAuthorityObservationDigest(value: Record<string, any>) {
...
export function protocolSignatureMessage(value: unknown) {
  const document = plainRecord(value, 'signedDocument')
  const signatureDomain = ((SIGNATURE_DOMAINS) as Record<string, string>)[document.kind]
  const documentDomain = ((RUNTIME_MANAGER_DOMAIN_TAGS) as Record<string, string>)[document.kind]
...
export function verifyProtocolSignatureDigest(input: {signedKind: string, rawDocumentDigest: string, keyId: string, rawPublicKeyHex: string, signature: string}) {
  const domain = ((SIGNATURE_DOMAINS) as Record<string, string>)[input.signedKind]
...
export function verifyProtocolSignature(value: unknown, bundle: unknown, expectedUse: 'seal'|'assertion'|'trust-transition') {
  const document = plainRecord(value, 'signedDocument')
  ...
  const key = trustBundle.keys.find((item: Record<string, any>) => item.keyId === documentKeyId)
...
export function validateTrustBundle(value: unknown) {
...
export function validateTrustLineageHead(value: unknown) {
...
export function validateAdmissionSeal(value: unknown, expected: {composition: unknown, compositionLockReceipt: unknown, identity: unknown}) {
  ...
  const composition = ((validateResolvedComposition(expected.composition)) as Record<string, any>)
  const lock = ((validateCompositionLockReceipt(expected.compositionLockReceipt, composition)) as Record<string, any>)
  ...
  for (const field of Object.keys(ceilings)) {
    if (authority[field].some((item: string) => !ceilings[field].includes(item))) fail('authority-widening', 'seal widens the public composition ceiling', { field })
  }
  if (composition.profile.scopeClass === 'non-project'
    && (authority.workspaceClasses.length > 0 || authority.capabilities.some((item: string) => item.startsWith('coding.')))) {
...
export function validateRuntimeAssertion(value: unknown, expected: {sealDigest: string, identity: unknown, operation: string, semanticRequestDigest: string}) {
...
function instanceState(value: unknown, path: string, nullable: boolean = false) {
...
function boundedStrings(value: unknown, path: string, limit: number = 1024) {
...
export function validateFailureDetails(value: unknown, code: string, path: string) {
...
function validateLifecycleReceipt(value: unknown) {
  ...
  const transition = ((MUTATION_TRANSITIONS) as Record<string, {sources: readonly string[], terminal: string}>)[receipt.operation]
  if (receipt.apiVersion !== RUNTIME_MANAGER_API_VERSION
    || transition === undefined
    || receipt.kind !== ((expectedKinds) as Record<string, string>)[receipt.operation]) {
...
function validateQuarantineReceipt(value: unknown) {
  ...
  const transition = ((MUTATION_TRANSITIONS) as Record<string, {sources: readonly string[]}>)[receipt.originalOperation]
  if (receipt.apiVersion !== RUNTIME_MANAGER_API_VERSION
...
export function validateRuntimeReceipt(value: unknown) {
...
function validateStatusFailure(value: unknown) {
...
function validateKnownRequest(payload: Record<string, any>) {
  const keys = ((REQUEST_KEYS) as Record<string, readonly string[]>)[payload.kind]
  ...
  if (payload.kind === 'ValidateCompositionRequest') {
    return ((validateCompositionProtocolRequest(payload)) as Record<string, any>)
  }
  ...
  if (payload.kind === 'ReconcileInstanceRequest') {
    const transition = ((MUTATION_TRANSITIONS) as Record<string, {sources: readonly string[], terminal: string}>)[payload.originalOperation]
  ...
  if (payload.kind === 'LockInstanceRequest') {
    ...
    const composition = ((validateResolvedComposition(payload.resolvedComposition)) as Record<string, any>)
    const lock = ((validateCompositionLockReceipt(payload.compositionLockReceipt, composition)) as Record<string, any>)
...
function validateManagerResult(payload: Record<string, any>) {
  if (payload.kind.startsWith('ValidateComposition') || payload.kind.startsWith('ResolveComposition')) {
    return ((validateCompositionProtocolResult(payload)) as Record<string, any>)
  }
  const successKeys = ((SUCCESS_KEYS) as Record<string, readonly string[]>)[payload.kind]
  if (successKeys !== undefined) {
    ...
    if (payload.kind === 'ReconcileInstanceIndeterminate') {
      ...
      if (((MUTATION_TRANSITIONS) as Record<string, any>)[payload.originalOperation] === undefined) fail('schema-invalid', 'reconcile indeterminate operation is invalid')
    ...
    if (payload.kind === 'StatusInstanceSucceeded') {
      ...
      if (((MUTATION_TRANSITIONS) as Record<string, any>)[payload.originalOperation] === undefined) fail('schema-invalid', 'quarantine result operation is invalid')
      text(payload.originalIdempotencyKey, `${payload.kind}.originalIdempotencyKey`, 256)
      digest(payload.originalRequestDigest, `${payload.kind}.originalRequestDigest`)
      ...
      const transition = ((MUTATION_TRANSITIONS) as Record<string, {terminal: string}>)[operation]
  ...
  if (payload.kind.endsWith('Indeterminate')) {
    ...
    const transition = ((MUTATION_TRANSITIONS) as Record<string, {transient: string}>)[operation]
  ...
  if (payload.kind.endsWith('Failed')) {
    ...
    const allowedCodes = ((MANAGER_FAILURE_CODES_BY_KIND) as Record<string, Set<string>>)[payload.kind]
...
function correlateManagerResult(request: Record<string, any>, result: Record<string, any>) {
  ...
  if (result.receipt !== undefined && request.requestDigest !== undefined) {
    ...
    const operation = ((MANAGER_OPERATION_BY_REQUEST) as Record<string, string>)[request.kind]
...
export function validateManagerPayload(value: unknown) {
...
export function createManagerControlRequestFrame(input: {connectionNonce: string, payload: unknown}) {
...
export function validateManagerControlRequestFrame(value: unknown) {
...
export function createManagerControlResponseFrame(input: {requestFrame: unknown, payload: unknown}) {
  const request = ((validateManagerControlRequestFrame(input.requestFrame)) as Record<string, any>)
  ...
  const allowed = ((MANAGER_RESULT_KINDS_BY_REQUEST) as Record<string, readonly string[]>)[request.payloadKind]
...
export function validateManagerControlResponseFrame(value: unknown, requestFrame: unknown) {
  assertCanonicalByteBound(value, 'ManagerControlResponseFrame')
  const request = ((validateManagerControlRequestFrame(requestFrame)) as Record<string, any>)
...
export function createManagerControlService(options: {
  ...
  const handle = async (value: unknown, context: {peerIdentity: string}) => {
    const frame = ((validateManagerControlRequestFrame(value)) as Record<string, any>)
    ...
    const operation = ((MANAGER_OPERATION_BY_REQUEST) as Record<string, string>)[frame.payloadKind]
