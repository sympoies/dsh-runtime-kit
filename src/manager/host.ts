...
export function validateMediatedHostActionRequest(value: unknown) {
...
export function validateMediatedHostActionResult(value: unknown) {
...
function validateExternalEffectReceipt(value: unknown) {
...
function safeFailureText(value: unknown, maximum: number) {
  try {
...
function safeFailureDigest(value: unknown) {
  try {
...
function safeFailureEpoch(value: unknown) {
...
function hostTrustFailureEvidence(error: unknown) {
...
function failed(request: Record<string, any>, code: string, state: string | null, acceptance: string | null = null, acceptanceFailure: string | null = null) {
...
function indeterminateEffect(request: Record<string, any>, state: string, acceptance: string, externalIdempotencyToken: string) {
  const result = {
...
export function createMediatedHostService(options: {
  ...
  const executeWithinLock = async (value: unknown) => {
    let request
    ...
    const row: Record<string, any> = { requestDigest: request.requestDigest, revision: '0', acceptanceDigest: null, result: null }
    journal.set(key, row)
    let acceptance
    try {
  ...
  const execute = async (value: unknown) => {
    const namespace = value !== null && typeof value === 'object'
      && typeof ((value) as any).identity?.namespace === 'string'
