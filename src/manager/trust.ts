...
const TRUST_STATE_COORDINATORS: WeakMap<Map<any, any>, Map<string, Promise<any>>> = new WeakMap()
...
function boundedDatagram(value: unknown, path: string) {
  assertCanonicalByteBound(value, path, MAX_DATAGRAM_BYTES)
}

function validateTransition(value: unknown) {
...
function validateTransitionEdge(transition: Record<string, any>, prior: Record<string, any>, next: Record<string, any>, authorityTime: string, tombstones: Set<string>) {
  ...
  if (transition.keyStateChanges.map((change: Record<string, any>) => change.keyId).join('\0')
    !== [...transition.keyStateChanges].map((change: Record<string, any>) => change.keyId).sort().join('\0')
  ...
  const overlapIds = transition.retirementOverlapEnds.map((overlap: Record<string, any>) => overlap.keyId)
...
function validateLineageSuccess(response: Record<string, any>, request: Record<string, any>, controllerIdentity: string) {
  exactKeys(response, [
...
function validateBundleSuccess(response: Record<string, any>, request: Record<string, any>, controllerIdentity: string, observationDigest: string) {
  exactKeys(response, [
    'apiVersion', 'kind', 'digest', 'requestId', 'requestDigest',
...
function validateAuthorityFailure(port: 'lineage'|'bundle'|'acceptance', response: Record<string, any>, request: Record<string, any>, controllerIdentity: string) {
  ...
  const allowedCodes = ((AUTHORITY_FAILURE_CODES) as Record<string, Set<string>>)[expectedKind]
  const retryableCodes = ((AUTHORITY_RETRYABLE_CODES) as Record<string, Set<string>>)[expectedKind]
...
function throwAuthorityFailure(label: string, response: Record<string, any>) {
...
export function createTrustVerifier(options: {
  ...
  const freshId = (prefix: string) => `${prefix}-${++requestSequence}`

  const serializeNamespace = async (namespace: string, action: () => Promise<any>) => {
  ...
  const readBundle = async (namespace: string, head: Record<string, any>, observationDigest: string, bundleDigest: string) => {
  ...
  const refreshCurrent = async (namespace: string) => {
    const bootstrap = options.bootstrap[namespace]
    ...
    let head: Record<string, any> | null = null
    let firstResponse: Record<string, any> | null = null
    let transitionCount = 0n
    let lastSequence: string | null = null
    let lastTransitionDigest: string | null = null
    ...
    let priorBundle: Record<string, any> | null = reuseCached ? baseBundles.get(expectedBundle) ?? null : null
    let baseTombstones = reuseCached ? retained.tombstones ?? new Set() : new Set()
    let stagedTombstones = new Set()
    const tombstoneView = () => ({
      has: (key: string) => baseTombstones.has(key) || stagedTombstones.has(key),
      add: (key: string) => { stagedTombstones.add(key); return stagedTombstones },
    })
    while (true) {
      const request = {
        apiVersion: INFRA_API_VERSION,
  ...
  const readCurrent = async (namespace: string) => serializeNamespace(namespace, async () => {
  ...
  const acceptSignedDocument = async (input: {namespace: string, acceptanceKind: 'seal'|'assertion', signedDocument: any, operation: string, semanticRequestDigest: string, expectedEffectJournalRevision: string}) => serializeNamespace(input.namespace, async () => {
