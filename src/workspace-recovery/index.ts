...
function exactArguments(args: unknown, expected: string[], HarnessError: (new (message:string, code:string) => Error) | undefined) {
...
function typedFailure(HarnessError: (new (message:string, code:string) => Error) | undefined, message: string, code: string) {
...
function renderResult(value: any) {
  ...
  const eligible = value.worktrees.filter((entry: any) => (
...
export function createWorkspaceRecoveryTools(client: {inspect(exec: import('@deepseek-ai/dsh-tools').ToolRunContext): Promise<any>, verifyHandoff(exec: import('@deepseek-ai/dsh-tools').ToolRunContext, path:string): Promise<any>}, HarnessError?: (new (message:string, code:string) => Error) | undefined): readonly ToolDefinition[]  {
  if (client === null
