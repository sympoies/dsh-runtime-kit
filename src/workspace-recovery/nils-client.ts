...
function safePath(value: unknown, absolute: boolean) {
...
function executionScope(exec: ToolRunContext, HarnessError: (new (message: string, code: string) => Error) | undefined) {
  ...
  return { cwd: ((cwd) as string) }
}

function failure(HarnessError: (new (message: string, code: string) => Error) | undefined, message: string, code: string) {
  if (HarnessError !== undefined) return new HarnessError(message, code)
  return Object.assign(new Error(message), { code })
}

function serialize(request: Record<string, unknown>, HarnessError: (new (message: string, code: string) => Error) | undefined) {
  let payload
...
function result(raw: unknown, action: 'inspect'|'verify-handoff') {
...
function nonnegativeInteger(value: unknown, optional: boolean = false) {
...
function parseCheckout(raw: unknown) {
  ...
  return Object.freeze({
    ...
    dirty_entries: ((dirtyEntries) as any[]),
    dirty_entries_omitted: value.dirty_entries_omitted ?? 0,
  })
}

function parseWorktree(raw: unknown) {
  const value = record(raw)
  if (value === undefined
    || !safePath(value.path, true)
    || (value.branch !== null && !safeText(value.branch))
...
function nilsFailureCode(raw: unknown, action: 'inspect'|'verify-handoff') {
  const envelope = record(raw)
...
export function createNilsWorkspaceRecoveryClient(ctx: Context, config: {agentHook?:string, agentHookConfig?:string, agentHookPolicy?:string, agentHookStateDir?:string, authenticatedNilsExecution?:unknown, workspaceRecoveryTimeoutMs?:number, workspaceRecoveryTeardownTimeoutMs?:number, maxActiveWorkspaceRecoveryRequests?:number, HarnessError?:new (message:string, code:string)=>Error} = {}) {
  ...
  function cancel(operation: any, cause: 'caller-aborted'|'timeout'|'disposed'|'degraded') {
  ...
  async function invoke(action: 'inspect'|'verify-handoff', request: Record<string, unknown>, signal: AbortSignal) {
    if (!open || degraded) {
    ...
    const operation = {
      ...
      handle: ((undefined) as SubprocessHandle | undefined),
    ...
    try {
      ...
      const first = await Promise.race([
        ...
        cancelled.then(() => ({ kind: (('cancelled') as const), outcome: undefined })),
      ])
      if (first.kind !== 'done') {
  ...
  return Object.freeze({
    async inspect(exec: ToolRunContext) {
    ...
    async verifyHandoff(exec: ToolRunContext, path: string) {
      const scope = executionScope(exec, HarnessError)
