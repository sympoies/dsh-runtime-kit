...
function validDefinition(definition: unknown) {
...
export function createPrerequisiteCoordinator(ctx: Context, client: {beginPrerequisite(exec: ToolExecution, intent: string, binding: ExecutionBinding): Promise<any>, commitPrerequisite(exec: ToolExecution, pending: {intent: string, phase: string, receipt: string, binding: ExecutionBinding}): Promise<any>}, createUserMessage: (input: any) => import('@deepseek-ai/dsh-llm').UserMessage, revalidatePolicy: (exec: ToolExecution, correlation: ToolCorrelation, proof: PrerequisiteProof) => Promise<{kind?: string, context?: string, reason?: string} | undefined>) {
  ...
  let definitionIds: WeakMap<ToolDefinition, string> = new WeakMap()
  let agentIds: WeakMap<Agent, string> = new WeakMap()
  const pending: Map<Readonly<ToolExecution>, PendingPrerequisite> = new Map()
  const workspaceGeneration = `workspace:${randomUUID()}`
  let open = true

  function definitionId(definition: ToolDefinition) {
  ...
  function visibleDefinition(exec: Readonly<ToolExecution>) {
  ...
  function requirement(exec: Readonly<ToolExecution>) {
  ...
  function recordMismatch(exec: Readonly<ToolExecution>, record: PendingPrerequisite) {
  ...
  function requireMatchingRecord(exec: Readonly<ToolExecution>, record: PendingPrerequisite) {
  ...
  async function commit(exec: Readonly<ToolExecution>, record: PendingPrerequisite) {
 ...
 */
    ...
    require(definition: ToolDefinition, capability: string) {
  ...
  return Object.freeze({
    ...
    attachAgent(_agent: Agent) {},
    detachAgent(_agent: Agent) {},
    prepare(_exec: ToolExecution) {},

    async begin(exec: ToolExecution, correlation: {sessionId: string, cwd: string, turn: number, step: number, callId: string, name: string}) {
      if (!open) throw new Error('dsh-runtime-kit:prerequisite-registry-disposed')
      ...
      try {
        const tools = ((ctx.tools) as Context['tools'] & {bindPrerequisite?: Function})
        if (typeof tools.bindPrerequisite !== 'function') {
          throw new Error('dsh-runtime-kit:prerequisite-native-seam-unavailable')
        }
        tools.bindPrerequisite(exec, required.definition, Object.freeze({
          beforeBody: (runtimeExec: ToolRunContext, phase: 'dispatch' | 'body') => (
            beforeBody(runtimeExec, record, phase)
          ),
          commit: (runtimeExec: Readonly<ToolExecution>) => commit(runtimeExec, record),
        }))
      } catch (error) {
        pending.delete(exec)
        throw error
    ...
    reject(exec: Readonly<ToolExecution>) { pending.delete(exec) },
    result(exec: Readonly<ToolExecution>) { pending.delete(exec) },
...
export type ExecutionBinding = { agentId: string, workspaceGeneration: string, callId: string, turn: number, step: number, toolName: string, definitionId: string }

export type ToolCorrelation = { sessionId: string, cwd: string, turn: number, step: number, callId: string, name: string }

export type PrerequisiteProof = { agentId: string, workspaceGeneration: string, definitionId: string, receipt: string }
