...
export type LaneTurnEvidence = { phase: 'working' | 'waiting', phaseChangedAt: string, currentTurn: { startedAt: string, lastProgressAt?: string } | undefined, lastTurn: { completedAt: string, outcome: string } | undefined }

export type Lane = { assignmentId: string, workerSessionId: string, launchId: string, livenessFile: string, childId: string, parent: unknown, worktree: string, state: 'open' | 'terminated', turn: LaneTurnEvidence | undefined, workerEnv: Readonly<Record<string, string>>, bootstrapKey: string, brokerStopArgv: readonly string[], stopHeartbeat: (() => void) | undefined, sidecarChain: Promise<void> }
...
let memoizedHarnessIdentity: { pid: number, start_time?: number } | undefined
...
export function harnessIdentity() {
  ...
  const identity: { pid: number, start_time?: number } = { pid: process.pid }
...
export function createLaneRegistry() {
  const byAssignment: Map<string, Lane> = new Map()
  const byChild: Map<string, Lane> = new Map()
  ...
  const byLivenessFile: Map<string, Lane> = new Map()
  const byWorkerSession: Map<string, Lane> = new Map()
  const byMember: Map<string, Lane> = new Map()
  return Object.freeze({
    ...
    byLivenessFile(livenessFile: string) {
      return byLivenessFile.get(resolve(livenessFile))
    },
    byWorkerSession(workerSessionId: string) {
      return byWorkerSession.get(workerSessionId)
    },
    /** Bind the DSH-reserved child before unpublished materialization. */

    bindChild(lane: Lane) {
      byChild.set(lane.childId, lane)
    },
    bindMember(sessionId: string, lane: Lane) {
      byMember.set(sessionId, lane)
    },
    unbindMember(sessionId: string) {
      byMember.delete(sessionId)
    },
    byMember(sessionId: string) {
