...
const SCENARIO_CANARY_DEADLINE_CAUSE_CODES = Object.freeze([
  ...
  'DSH_CANARY_DEADLINE_VALIDATION_TOOL_REQUESTED',
  'DSH_CANARY_DEADLINE_VALIDATION_TOOL_RESULT',
  'DSH_CANARY_DEADLINE_HOST_VALIDATOR_REQUESTED',
  'DSH_CANARY_DEADLINE_HOST_VALIDATOR_RESULT',
  'DSH_CANARY_DEADLINE_STOP_REQUESTED',
  'DSH_CANARY_DEADLINE_TURN_STOPPING_ENTERED',
  ...
  'DSH_CANARY_DEADLINE_CANARY_REPEATED_STOP_RESERVATION_UNTERMINALIZED',
...
// Typed codes of the repository-owned deploy dispatcher (src/deploy/index.js),
// which the operations scenario drives through `.agents/scripts/deploy.sh`.
const DEPLOY_SCENARIO_CAUSE_CODES = Object.freeze([
  ...
  'unexpected-plan-digest',
].map(code => `DSH_DEPLOY_${code.replaceAll('-', '_').toUpperCase()}`))
const SCENARIO_CAUSE_CODES = new Set([
  ...OPERATIONS_SCENARIO_CAUSE_CODES,
  ...DEPLOY_SCENARIO_CAUSE_CODES,
...
function exactVerdict(value: unknown, action: 'allow'|'block', aggregate: string) {
...
function zeroResources(value: unknown) {
...
function exactPatchTransition(value: unknown, action: 'apply'|'reverse') {
  const selected = exactRecord(value, [
...
function exactBuildClosure(value: unknown) {
...
function authoritativeDshLifecycle(value: unknown) {
...
function commonAuthoritativeLeg(leg: Record<string, any>) {
...
function authoritativeMatrix(value: unknown): Readonly<Record<string, any>>  {
  ...
  const rawLegs = ((matrix.legs) as Array<Record<string, any>>)
  const legs: Array<Readonly<Record<string, any>>> = rawLegs.map((raw, index) => {
...
function scenarioIsolation(item: Record<string, any>, producer: 'operations'|'packed-runtime') {
...
function requiredScenarioEvidence(producer: 'operations'|'packed-runtime', id: string) {
...
export function createScenarioFailureDiagnosticTracker(producer: unknown) {
  let step = 'input-authentication'
  let operationExitStatus: number | undefined
  let operationCauseCode: string | undefined
  let operationSignal: string | undefined
  let taken = false
  return Object.freeze({
    enterStep(nextStep: unknown) {
    ...
    recordOperationExitStatus(nextStatus: unknown) {
 ...
 */
    ...
    recordOperationOutcome(nextStatus: unknown, nextCauseCode: unknown, nextSignal: unknown) {
      operationExitStatus = typeof nextStatus === 'number'
        && Number.isSafeInteger(nextStatus)
        && nextStatus >= 1
        && nextStatus <= 255
...
export function parseScenarioCanaryReceipt(input: {
  ...
  const enter = (suffix: string) => input.enterStep(`${phase}-receipt-${suffix}`)
  enter('marker')
...
export async function waitForScenarioOperationMarker(input: {
  ...
  let spawnError
  const onError = (error: unknown) => { spawnError = error }
...
export function scenarioFailureDiagnostic(output: unknown) {
...
function exactArtifacts(artifacts: Record<string, any>, names: ReadonlyArray<string> = CANDIDATE_NILS_ARTIFACTS) {
...
function sameArtifacts(left: Record<string, any>, right: Record<string, any>, names: ReadonlyArray<string> = CANDIDATE_NILS_ARTIFACTS) {
  return exactArtifacts(left, names)
    && exactArtifacts(right, names)
    && names.every(name => left[name].sha256 === right[name].sha256)
}

export function resolveSourceCandidateAcceptance(compatibilityValue: unknown, nilsValue: unknown) {
...
function scenariosFrom(input: unknown, producer: 'operations'|'packed-runtime', dataPolicyCandidateEnabled: boolean = false) {
...
function dshAccepted(actual: Record<string, any>, expected: Record<string, any>) {
...
function releaseAccepted(compatibility: Record<string, any>, nils: Record<string, any>) {
...
function repositorySlug(repository: string) {
  const match = repository.match(REPOSITORY)
  return match === null ? undefined : match[1] + '/' + match[2]
}

function deliveryBindingAccepted(delivery: Record<string, any>, runId: string, expected: Record<string, any>) {
...
function semanticCommitAccepted(delivery: Record<string, any>, runId: string, expected: Record<string, any>) {
...
function prDeliveryAccepted(delivery: Record<string, any>, runId: string, expected: Record<string, any>) {
  if (!deliveryBindingAccepted(delivery, runId, expected)) return false
  const receipt = delivery.pr_delivery
  const data = receipt?.data
  const steps = Array.isArray(data?.steps)
    ? ((data.steps) as Array<Record<string, any>>)
    : []
...
export function buildAcceptanceSummary(input: {
  ...
  const frozenScenarios = ((scenarios) as Array<Record<string, any>>)
...
export function buildAcceptanceCliResult(summary: Record<string, any>) {
