...
export type ArtifactProvider = { capabilities: readonly ArtifactCapability[], root?: string, init: () => Promise<void>, list: () => Promise<readonly ArtifactRecord[]>, load: (id: string) => Promise<ArtifactRecord | undefined>, begin: (draft: {id: string, maxBytes: number}) => Promise<ArtifactStaging>, publish: (record: ArtifactRecord) => Promise<void>, read: (record: ArtifactRecord, signal?: AbortSignal) => Promise<Uint8Array>, remove: (record: ArtifactRecord) => Promise<void>, removeMany: (records: readonly ArtifactRecord[]) => Promise<void>, claimGeneration: (generation: string) => Promise<void>, releaseGeneration: (generation: string) => Promise<void>, generationAlive: (generation: string) => Promise<boolean>, listGenerations: () => Promise<readonly string[]> }
/**
...
export type ArtifactStaging = { write: (chunk: Uint8Array) => Promise<void>, commit: () => Promise<{sha256: string, bytes: number}>, abort: () => Promise<void> }
/**
...
function corrupt(message: string, cause?: unknown) {
  return new ArtifactError(describe(message, cause), ARTIFACT_CODES.CORRUPT)
}

function writeFailed(message: string, cause?: unknown) {
...
function ownedByCurrentUser(metadata: import('node:fs').Stats) {
...
export class LocalArtifactProvider {
  capabilities: readonly ArtifactCapability[] = Object.freeze(((['read', 'present', 'export', 'delete']) as const))
  root: string
  #initialized = false
  ...
  async begin(draft: {id: string, maxBytes: number}) {
    this.#requireInit()
    ...
    const objectPath = (sha256: string) => this.#objectPath(sha256)
    const discard = async () => {
      if (handle !== undefined) {
  ...
  async read(record: ArtifactRecord, signal?: AbortSignal) {
    this.#requireInit()
 ...
 */
  ...
  async removeMany(records: readonly ArtifactRecord[]) {
    this.#requireInit()
 ...
 */
  ...
  async claimGeneration(generation: string) {
    this.#requireInit()
  ...
  async releaseGeneration(generation: string) {
    this.#requireInit()
    const path = this.#generationPath(generation)
 ...
 */
  ...
  async generationAlive(generation: string) {
    this.#requireInit()
    const path = this.#generationPath(generation)
...
async function verifyObject(target: string, sha256: string, bytes: number) {
