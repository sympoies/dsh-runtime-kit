...
export type ArtifactService = import('./index.js').ArtifactService
...
function exactArguments(args: unknown, required: readonly string[], optional: readonly string[] = []) {
...
function requireRef(value: unknown) {
...
function executingAgent(exec: ToolRunContext) {
...
function toolProjection(projection: import('./record.js').ArtifactProjection) {
  return {
    ref: projection.ref,
...
function renderRecord(value: Record<string, unknown>) {
  const lines = [
...
export function createArtifactTools(service: ArtifactService): readonly ToolDefinition[]  {
  ...
  const write: ToolDefinition = {
    ...
    output: {
      ...
      render: (_args, value) => renderRecord(((value) as Record<string, unknown>)),
    },
    async execute(args, exec) {
      ...
      if (parsed.retention !== undefined && (typeof parsed.retention !== 'string' || !RETENTION_CLASSES.includes(((parsed.retention) as any)))) {
      ...
      const projection = await service.write(executingAgent(exec), {
        ...
        ...(parsed.retention === undefined ? {} : { retention: ((parsed.retention) as any) }),
  ...
  const read: ToolDefinition = {
    ...
    async execute(args, exec) {
      ...
      let encoding = (('base64') as 'utf8' | 'base64')
