...
function hasManagedSessionCandidate(environment: Readonly<NodeJS.ProcessEnv>) {
...
function selectCandidateEnvironment(environment: Readonly<NodeJS.ProcessEnv>) {
...
function validPublicWorkContext(value: unknown) {
...
function validConflictEvaluation(value: unknown) {
  if (!isRecord(value)) return false
  const evaluation = ((value) as Record<string, any>)
...
function validBaselineClaim(envelope: unknown, candidate: Readonly<Record<string, string>>) {
...
function baselineScopeUnavailableCode(envelope: unknown) {
  ...
  return outer.schema_version === WORK_CONTEXT_SET_ENVELOPE_SCHEMA
    ...
    ? ((outer.error.code) as 'not-in-repository' | 'repository-unavailable' | 'uncovered-mutation-scope')
    : undefined
}

function remainingAuthenticationMs(deadlineAt: number) {
...
export function applyManagedSessionAuthentication(
  ...
  const disposedAgents: WeakSet<object> = new WeakSet()
  let closing = false

  const trustedAgentSessionCli = async (signal: AbortSignal | undefined) => {
  ...
  const authenticate = async (providerSessionId: string, exec: any) => {
