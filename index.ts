...
export type FileSystemSkillProvider = import('@deepseek-ai/dsh-skill-filesystem').FileSystemSkillProvider

export type Context = import('@deepseek-ai/cordis').Context
export type Stats = import('node:fs').Stats

export type PatchedContext = Context & {
    llm: Context['llm'] & {
...
export type PrivateLimitOptions = { maxDepth?: number, maxEntries?: number }
...
function configuredPrivateSkillsDir(config: RuntimeKitConfig) {
...
function artifactLimits(config: RuntimeKitConfig) {
...
function boundedLimit(value: unknown, fallback: number, hardMaximum: number, field: string) {
...
function privateLimits(options: PrivateLimitOptions = {}) {
  return {
...
function writableByGroupOrWorld(metadata: Stats) {
  return (metadata.mode & 0o022) !== 0
}

function assertCurrentUserOwner(metadata: Stats, subject: string) {
  if (typeof process.getuid === 'function' && metadata.uid !== process.getuid()) {
    throw new Error(`dsh-runtime-kit: ${subject} must be owned by the current user`)
  }
}

async function assertTrustedAncestor(directory: string) {
...
function entryFingerprint(path: string, type: 'file' | 'directory', metadata: Stats): PrivateEntry  {
  return {
    path,
    type,
    dev: metadata.dev,
...
function sameFingerprint(expected: PrivateEntry, metadata: Stats) {
...
async function trustedPrivateTree(root: string, limits: { maxDepth: number, maxEntries: number }): Promise<PrivateTree>  {
...
function sameTree(left: PrivateTree, right: PrivateTree) {
...
function containedDestination(root: string, child: string) {
...
async function copyPrivateTree(observation: PrivateTree, snapshotRoot: string) {
...
function findClosingFrontmatter(raw: string, start: number) {
  let lineStart = start
...
function frontmatterBoolean(data: Record<string, unknown>, key: string) {
...
function parsePrivateSkill(raw: string, path: string, resourcePath: string) {
...
function privateDefinitions(observation: PrivateTree, snapshotRoot: string, contents: Map<string, Buffer>) {
  const definitions = []
  const names = new Set()
...
async function sealSnapshot(observation: PrivateTree, snapshotRoot: string) {
  for (const entry of observation.entries.filter(candidate => candidate.type === 'file')) {
...
async function removeSnapshot(observation: PrivateTree, snapshotRoot: string) {
  try {
...
export async function apply(ctx: PatchedContext, config: RuntimeKitConfig = {}) {
  ...
  try {
    ...
    const configuredRuntime: RuntimeKitConfig = { ...config, managedSessionBridge }
