...
async function run() {
  ...
  await waitForBrokerReady(launchedOverlap)
  ...
  /**
   * Block until the lane's coordination broker reports ready. Readiness is
   * established by the lane's own heartbeat, so this is a real wait on a real
   * process, bounded so a broken lane fails loudly instead of hanging.
   */
  async function waitForBrokerReady(lane) {
    ...
    while (Date.now() < deadline) {
      ...
      if (result.status === 0) {
        ...
        if (status.state === 'ready' && status.heartbeat_fresh !== false) return status
      }
      await new Promise(resolve => setTimeout(resolve, 250))
    }
    assert.fail(`lane ${lane.assignment_id} broker never became ready: ${last}`)
  }
