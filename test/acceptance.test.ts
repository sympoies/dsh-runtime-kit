...
test('authoritative acceptance rejects fabricated matrix identities and observations', () => {
  const mutations = [
    ...
    matrix => { matrix.legs[0].turn_stops = 2 },
    matrix => { matrix.legs[0].goal_round_followups = 1 },
    ...
    matrix => { matrix.legs[8].goal_round_followups = 1 },
...
test('acceptance runner is packaged with its scenario programs and rejects old receipt injection flags', async () => {
  ...
  assert.deepEqual(
    ...
    [
      ...
      '../dist/src/compat/dsh-tui-pristine.js',
      '../dist/src/compat/dsh-tui-history.js',
