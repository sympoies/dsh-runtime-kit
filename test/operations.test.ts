...
`)
    ...
    assert.match(old.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.43/)
...
`)
    ...
    writeFileSync(subject.agentDocs, `#!/usr/bin/env node
if (process.argv.length !== 3 || process.argv[2] !== '--version') process.exit(91)
process.stdout.write('agent-docs 1.28.43 (v1.28.43, test)\\n')
`)
    chmodSync(subject.agentDocs, 0o755)
    const validatedLatest = run(subject, ['doctor', '--profile', 'work'])
    assert.equal(validatedLatest.status, 0, validatedLatest.stderr)
    assert.equal(validatedLatest.value.data.agent_docs.version, '1.28.43')
...
`)
    ...
    assert.match(newer.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.43/)
...
`)
    ...
    assert.match(oldNils.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.43/)
...
`)
    ...
    assert.match(previousNils.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.43/)
...
`)
    ...
    assert.equal(currentNils.value.data.agent_docs.version, '1.28.43')
...
`)
    ...
    assert.match(retained.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.43/)
  } finally {
    subject.cleanup()
  }
})
