...
`)
    ...
    assert.match(old.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.23/)
...
`)
    ...
    assert.equal(validatedLatest.value.data.agent_docs.version, '1.28.23')

    writeFileSync(subject.agentDocs, `#!/usr/bin/env node
if (process.argv.length !== 3 || process.argv[2] !== '--version') process.exit(91)
process.stdout.write('agent-docs 1.28.24 (v1.28.24, test)\\n')
`)
    chmodSync(subject.agentDocs, 0o755)
    const newer = run(subject, ['doctor', '--profile', 'work'])
    assert.equal(newer.status, 65)
    assert.equal(newer.value.data.agent_docs.ok, false)
    assert.match(newer.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.23/)
...
`)
    ...
    assert.match(previousNils.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.23/)
...
`)
    ...
    assert.equal(currentNils.value.data.agent_docs.version, '1.28.23')
