...
`)
    ...
    assert.match(old.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.25/)
...
`)
    ...
    writeFileSync(subject.agentDocs, `#!/usr/bin/env node
if (process.argv.length !== 3 || process.argv[2] !== '--version') process.exit(91)
process.stdout.write('agent-docs 1.28.25 (v1.28.25, test)\\n')
`)
    chmodSync(subject.agentDocs, 0o755)
    const validatedLatest = run(subject, ['doctor', '--profile', 'work'])
    assert.equal(validatedLatest.status, 0, validatedLatest.stderr)
    assert.equal(validatedLatest.value.data.agent_docs.version, '1.28.25')

    writeFileSync(subject.agentDocs, `#!/usr/bin/env node
if (process.argv.length !== 3 || process.argv[2] !== '--version') process.exit(91)
process.stdout.write('agent-docs 1.28.26 (v1.28.26, test)\\n')
`)
    chmodSync(subject.agentDocs, 0o755)
    const newer = run(subject, ['doctor', '--profile', 'work'])
    assert.equal(newer.status, 65)
    assert.equal(newer.value.data.agent_docs.ok, false)
    assert.match(newer.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.25/)
...
`)
    ...
    assert.match(oldNils.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.25/)
...
`)
    ...
    assert.match(previousNils.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.25/)
...
`)
    ...
    assert.equal(currentNils.value.data.agent_docs.version, '1.28.25')
...
`)
    ...
    assert.match(retained.value.data.agent_docs.error, /supported range 1\.28\.3 through 1\.28\.25/)
  } finally {
    subject.cleanup()
  }
})
