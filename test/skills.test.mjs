...
test('mandatory workflow policy references resolve to DSH-owned public resources', () => {
  ...
  assert.match(upstreamPolicy, /Configuration or a supported extension point/u)
  assert.match(upstreamPolicy, /public API or\s+schema/u)
  assert.match(upstreamPolicy, /CONTRIBUTING\.md.*CODE_OF_CONDUCT\.md.*SECURITY\.md/su)
  assert.match(upstreamPolicy, /merged to the default branch but not yet released/u)
  assert.match(upstreamPolicy, /accepted submission as an ongoing obligation/u)
})
