...
`)
    ...
    assert.match(old.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.32/)

    writeFileSync(subject.agentDocs, `#!/usr/bin/env node
if (process.argv.length !== 3 || process.argv[2] !== '--version') process.exit(91)
process.stdout.write('agent-docs 1.27.32 (v1.27.32, test)\\n')
`)
    chmodSync(subject.agentDocs, 0o755)
    const validatedCurrent = run(subject, ['doctor', '--profile', 'work'])
    assert.equal(validatedCurrent.status, 0, validatedCurrent.stderr)
    assert.equal(validatedCurrent.value.data.agent_docs.version, '1.27.32')
...
`)
    ...
    writeFileSync(subject.agentDocs, `#!/usr/bin/env node
if (process.argv.length !== 3 || process.argv[2] !== '--version') process.exit(91)
process.stdout.write('agent-docs 1.27.33 (v1.27.33, test)\\n')
`)
    chmodSync(subject.agentDocs, 0o755)
    const newer = run(subject, ['doctor', '--profile', 'work'])
    assert.equal(newer.status, 65)
    assert.equal(newer.value.data.agent_docs.ok, false)
    assert.match(newer.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.32/)
...
`)
    ...
    assert.match(oldNils.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.32/)
...
`)
    ...
    assert.match(previousNils.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.32/)
...
`)
    ...
    assert.match(retained.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.32/)
  } finally {
    subject.cleanup()
  }
})
