...
`)
    ...
    assert.match(old.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.21/)

    writeFileSync(subject.agentDocs, `#!/usr/bin/env node
if (process.argv.length !== 3 || process.argv[2] !== '--version') process.exit(91)
process.stdout.write('agent-docs 1.27.21 (v1.27.21, test)\\n')
`)
    chmodSync(subject.agentDocs, 0o755)
    const validatedCurrent = run(subject, ['doctor', '--profile', 'work'])
    assert.equal(validatedCurrent.status, 0, validatedCurrent.stderr)
    assert.equal(validatedCurrent.value.data.agent_docs.version, '1.27.21')
...
`)
    ...
    assert.match(newer.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.21/)
...
`)
    ...
    assert.match(oldNils.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.21/)
...
`)
    ...
    assert.match(previousNils.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.21/)
...
`)
    ...
    assert.match(retained.value.data.agent_docs.error, /supported range 1\.27\.17 through 1\.27\.21/)
  } finally {
    subject.cleanup()
  }
})
